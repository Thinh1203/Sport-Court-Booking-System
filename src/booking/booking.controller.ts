...
export class BookingController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Get my history booking successfully',
  })
  @ApiResponse({ status: 404, description: 'User not found' })
  @ApiResponse({ status: 400, description: 'Error' })
  @ApiBearerAuth()
  async getBookingByUserId(@Res() res: Response, @Req() req: Response) {
    try {
      const user = req['user'];
      const data = await this.bookingService.getBookingByUserId(
        Number(user.id),
      );
  ...
  async getAllBooking(@Query() query: BookingFilterDto, @Res() res: Response) {
    try {
